...
class GhostLogger {
    constructor(options) {
        ...
        this.stackdriver = options.stackdriver || {};
    ...
    setStackdriverStream() {
        const LoggingBunyan = require('@google-cloud/logging-bunyan');

        const stackdriverArgs = pick(this.stackdriver, ['projectId', 'keyFilename']);
        const stackdriverLog = LoggingBunyan(stackdriverArgs);

        this.streams.stackdriver = {
            name: 'stackdriver',
            match: this.stackdriver.match,
            log: bunyan.createLogger({
                name: 'Log',
                streams: [stackdriverLog.stream(this.stackdriver.level)],
                serializers: this.serializers
            })
        };
    }
